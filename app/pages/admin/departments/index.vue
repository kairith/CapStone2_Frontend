<template>
    <div class="departments-page">
                ...
                <div class="title-section">
                    <div class="title-wrapper">
                        <div class="title-icon">
                            <v-icon icon="mdi-office-building" size="32" color="white" />
                        </div>
                    ...
                    <div class="stats-cards">
                        ...
                        <div class="stat-card">
                            <div class="stat-number">{{ activeDepartments.length }}</div>
                            <div class="stat-label">Active</div>
                        </div>
                        <div class="stat-card">
                            <div class="stat-number">{{ inactiveDepartments.length }}</div>
            ...
            <div class="table-container">
                ...
                <div class="modern-table-wrapper">
                    <v-table class="modern-table">
                        <thead>
                            <tr class="modern-header-row">
                                ...
                                <th class="modern-header-cell center-align">
                                    <div class="header-content">Department Name</div>
                                </th>
                                <th class="modern-header-cell center-align">
                        ...
                        <tbody>
                            <tr v-for="(department, index) in paginatedDepartments" :key="department.id" class="modern-table-row">
                                ...
                                <td class="modern-table-cell center-align">
                                    <div class="code-badge">{{ department.global_id }}</div>
                                </td>
                                <td class="modern-table-cell center-align">
                                ...
                                <td class="modern-table-cell center-align">
                                    <v-chip :color="department.active === 1 ? 'success' : 'error'" size="small"
                                        class="status-chip">
                                        <v-icon start size="16">{{ department.active === 1 ? 'mdi-check-circle' : 'mdi-close-circle' }}</v-icon>
                                        {{ department.active === 1 ? 'Active' : 'Inactive' }}
                                    </v-chip>
                                </td>
                                <td class="modern-table-cell center-align">
                                    <div class="date-info">{{ formatDate(department.created_at) }}</div>
                                </td>
                                <td class="modern-table-cell center-align">
                                    <div class="action-group">
                                        <v-btn icon class="action-btn" @click="handleView(department)">
                                            <v-icon color="#3b82f6">mdi-eye</v-icon>
                                        </v-btn>
                                        <v-btn icon class="action-btn" @click="openEditDialog(department)">
                                            <v-icon color="#fde047">mdi-pencil</v-icon>
                                        </v-btn>
                                        <v-btn icon class="action-btn" @click="confirmDelete(department)">
                    ...
                    <div v-if="filteredDepartments.length === 0" class="empty-state">
                        <v-icon icon="mdi-office-building-outline" size="64" color="grey-lighten-1" />
                        <h3 class="empty-title">No departments found</h3>
                        <p class="empty-subtitle">
                            Create your first department to get started with academic structure management.
                        </p>
        ...
        <v-dialog v-model="viewDialog" max-width="800" persistent>
            <v-card class="modern-dialog" elevation="24" v-if="selectedDepartment">
                <!-- Dialog Header -->
                <div class="dialog-header">
                    <div class="header-content">
                        ...
                        <div class="header-text">
                            <h2 class="dialog-title">Department Details</h2>
                            <p class="dialog-subtitle">{{ selectedDepartment.name }}</p>
                        </div>
                    </div>
                ...
                <v-card-text class="dialog-content">
                    <div class="class-details">
                        ...
                        <div class="detail-section">
                            ...
                            <div class="detail-grid">
                                <div class="detail-item">
                                    ...
                                    <div class="detail-value">{{ selectedDepartment.id }}</div>
                                </div>
                                <div class="detail-item">
                                    <div class="detail-label">Global ID</div>
                                    <div class="detail-value">{{ selectedDepartment.global_id }}</div>
                                </div>
                                <div class="detail-item">
                                    <div class="detail-label">Department Name</div>
                                    <div class="detail-value">{{ selectedDepartment.name }}</div>
                                </div>
                                <div class="detail-item">
                                    <div class="detail-label">Status</div>
                                    <div class="detail-value">
                                        <v-chip :color="selectedDepartment.active === 1 ? 'success' : 'error'" size="small">
                                            {{ selectedDepartment.active === 1 ? 'Active' : 'Inactive' }}
                                        </v-chip>
                                    </div>
                                </div>
                                <div class="detail-item">
                                    <div class="detail-label">Created At</div>
                                    <div class="detail-value">{{ formatDate(selectedDepartment.created_at) }}</div>
                                </div>
                                <div class="detail-item">
                                    <div class="detail-label">Updated At</div>
                                    <div class="detail-value">{{ formatDate(selectedDepartment.updated_at) }}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="detail-section">
                            ...
                            <div class="detail-grid">
                                <div class="detail-item">
                                    <div class="detail-label">Total Specializations</div>
            ...
            <v-card class="modern-dialog" elevation="24">
                ...
                <div class="dialog-header">
                    <div class="header-content">
                        ...
                        <div class="header-text">
                            <h2 class="dialog-title">{{ isEdit ? 'Edit Department' : 'Add New Department' }}</h2>
                            <p class="dialog-subtitle">{{ isEdit ? 'Update department information' : 'Create a new department' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="warning-box">
                        ...
                        <div class="warning-text">
                            <p class="warning-message">
                                ...
                                <strong class="department-name">{{ selectedDepartment?.name }}</strong>
                            </p>
                        </div>
                    </div>
                </v-card-text>
...
const selectedDepartment = ref<Record<string, any> | null>(null)
...
// Computed properties
const activeDepartments = computed(() => departments.value.filter(d => d.active === 1))
const inactiveDepartments = computed(() => departments.value.filter(d => d.active === 0))
...
const filteredDepartments = computed(() => {
    ...
    if (searchQuery.value) {
        ...
        result = result.filter(d => 
            ...
            d.global_id.toLowerCase().includes(query)
        )
    }
...
const globalIdRules = [
    ...
    (v: string) => /^DEPT-\d{3}$/.test(v) || 'Global ID must be in format: DEPT-001'
]

const nameRules = [
    (v: string) => !!v || 'Department name is required',
    (v: string) => v.length >= 3 || 'Department name must be at least 3 characters',
    (v: string) => v.length <= 100 || 'Department name must not exceed 100 characters'
]
        ...
        if (isEdit.value && selectedDepartment.value) {
            ...
            if (index !== -1) {
                ...
                departments.value[index] = {
                    id: existingDept.id,
                    global_id: existingDept.global_id,
                    name: formData.name,
                    active: formData.active,
                    created_at: existingDept.created_at,
...
const generateNextGlobalId = () => {
    if (departments.value.length === 0) return 'DEPT-001'
    
    const ids = departments.value
        .map(d => {
            const parts = d.global_id.split('-')
