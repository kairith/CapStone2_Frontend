...
export class GenerationService {
  ...
  async update(id: number, data: Partial<GenerationFormData>): Promise<Generation> {
  ...
  async getCurrent(): Promise<Generation | null> {
    const response = await api.get<Generation | null>(`${this.baseURL}/current`);
    return response.data;
  }

  async getStudentCount(id: number): Promise<number> {
    const response = await api.get<{ count: number }>(`${this.baseURL}/${id}/student-count`);
